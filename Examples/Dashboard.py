...
temp_val = alte.Label(parent=w_temp, x=4, y=5, text='00' + units, font=lv.font_montserrat_48,
...
temp_pres = alte.Label(parent=w_temp, x=7, y=98, text='0000 hPa', font=lv.font_montserrat_18,
                       text_color=w_colors['font'])
...
remote_title = alte.Label(parent=w_remote, x=10, y=5, text='Remote\nM5stack', font=lv.font_montserrat_22,
                          text_color=w_colors['font'])
remote_disc = alte.Label(parent=w_remote, x=6, y=72, text='Temp: 00' + units, font=lv.font_montserrat_18,
                         text_color=w_colors['font'])
remote_pres = alte.Label(parent=w_remote, x=7, y=95, text='0000 hPa', font=lv.font_montserrat_18,
                         text_color=w_colors['font'])
...
def manage_settings():
    ...
    refresh_colors(theme_dict[
                       s_theme_drop_option])  # if s_units_switch_option:  #     units = 'F'  #     if units_old != units:  #         temp_val.set_text(str(int(C_to_F(local_temp))) + units)  #         remote_disc.set_text(str(int(C_to_F(remote_temp))) + units)  #         g_temp.set_range(round(C_to_F(0)), round(C_to_F(50)))  #         g_temp.set_critical_value()  # else:  #     units = 'C'  #     if units_old != units:  #         temp_val.set_text(str(int(F_to_C(local_temp))) + units)  #         remote_disc.set_text(str(int(F_to_C(remote_temp))) + units)  #         g_temp.set_range(0,50)  #         g_temp.set_critical_value()  #         g_temp.set_value(int(F_to_C(local_temp)))  # g_drop.set_options("\n".join(['Local Temp (' + units +')', 'Remote Temp (' + units +')', 'Local Pressure (kPa)', 'Remote Pressure (kPa)']))  # g_temp.set_hidden(False)  # g_pres.set_hidden(True)
...
s_cont = alte.Container(x=0, y=t.get_height(), height=LV_VER_RES - t.get_height(), width=LV_HOR_RES,
...
s_theme_label = alte.Label(parent=s_cont, x=5, y=50, text='Theme: ', font=lv.font_montserrat_22,
                           text_color=w_colors['font'])
s_theme_drop = alte.Dropdown(parent=s_cont, x=100, y=45, options=['Original', 'Green', 'Light', 'Dark'],
                             color=w_colors['dark_bg'])
s_refresh_label = alte.Label(parent=s_cont, x=5, y=85, text='Major refresh rate: ', font=lv.font_montserrat_22,
                             text_color=w_colors['font'])
s_refresh_drop = alte.Dropdown(parent=s_cont, x=220, y=80, width=75, options=['2 min', '5 min', '10 min', '20 min'],
                               color=w_colors['dark_bg'])
...
s_save = alte.Button(parent=s_cont, x=110, y=165, text='Save', color=w_colors['dark_bg'], height=35, width=100,
...
g_cont = alte.Container(x=0, y=t.get_height(), height=LV_VER_RES - t.get_height(), width=LV_HOR_RES,
                        color=w_colors['light_bg'], radius=0)
g_drop = alte.Dropdown(parent=g_cont, x=35, y=5, width=250,
                       options=['Local Temp (' + units + ')', 'Remote Temp (' + units + ')', 'Local Pressure (kPa)',
                                'Remote Pressure (kPa)'], color=w_colors['dark_bg'])
...
def fetch_data(topic_data):
    ...
    if alert_flag:
        alert_flag = False
        a = altw.Alert(text='BPS Has Detected Abnormal Temperature',
