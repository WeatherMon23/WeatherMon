...
def reset_charts(new_temp, new_pres, new_humid, is_click):
    ...
    temp_chart.delete()
    pres_chart.delete()
    temp_chart = Chart(parent = m_cont, x=60, y=94, height=110, width=200, min_val=0, max_val=50, input_vector=temp_reads, chart_type=lv.chart.TYPE.LINE)
    pres_chart = Chart(parent=m_cont, x=60, y=94, height=110, width=200, min_val=900, max_val=1100, input_vector=pres_reads, chart_type=lv.chart.TYPE.LINE)
...
m_cont = Page(x=0, y=t.get_height(), height=LV_VER_RES - t.get_height(), width=LV_HOR_RES, color=global_themes[theme_index]['light'], radius=0)
temp_l = Label(parent=m_cont, x=4, y=4, text=str('BPS Temperature: 21 ' + units), font=lv.font_montserrat_18, text_color=global_themes[theme_index]['font'])
pres_l = Label(parent=m_cont, x=4, y=29, text=str('BPS Pressure: 1007.3 hPa'), font=lv.font_montserrat_18, text_color=global_themes[theme_index]['font'])
refresh_btn = Button(parent=m_cont, x=260, y=4, text=lv.SYMBOL.LOOP, height=45, width=45, color=global_themes[theme_index]['dark'], font=lv.font_montserrat_30)
...
dropdown = Dropdown(parent= m_cont, x=85, y=59, width=150, options=drop_options, color=global_themes[theme_index]['dark'])
temp_chart = Chart(parent=m_cont, x=60, y=94, height=110, width=200, min_val=0, max_val=50, input_vector=temp_reads, chart_type=lv.chart.TYPE.LINE)
pres_chart = Chart(parent=m_cont, x=60, y=94, height=110, width=200, min_val=900, max_val=1100, input_vector=pres_reads, chart_type=lv.chart.TYPE.LINE)
...
def send_summary_email():
    ...
    # TODO (Ahmad): Send summarized_data via Email.
...
def export_event_handler(source,evt):
    ...
    if evt == lv.EVENT.CLICKED:
        if not sum_list:
            empty_list_alert = Alert('There is no data to export!', text_color=global_themes[theme_index]['font'], color=global_themes[theme_index]['light'])
            return
        ...
        for entry in sum_list:
            ...
            summarized_data += str(i) + ') ' + str(entry[4]) + ' [' + entry_type + '] - Temperature: ' + str(entry[0]) + ' ' + units + ', Pressure: ' + str(entry[1]) + ' hPa' + ', Humidity: ' + str(entry[2]) + ' RH\n'
            i += 1
        summary_confirm = Confirmation(text = 'Are you sure you want to export the data?',
                                       text_color=global_themes[theme_index]['font'],
                                       color=global_themes[theme_index]['light'],
                                       confirm_func=send_summary_email)

s_cont = Page(x=0, y=t.get_height(), height=LV_VER_RES - t.get_height(), width=LV_HOR_RES, color=global_themes[theme_index]['light'], radius=0)
s_cont.set_hidden(True)
s_title = Label(parent=s_cont, x=5, y=5, text=lv.SYMBOL.SETTINGS + ' Settings', font=lv.font_montserrat_30, text_color=global_themes[theme_index]['font'])
s_theme_label = Label(parent=s_cont, x=5, y=50, text='Theme: ', font=lv.font_montserrat_22, text_color=global_themes[theme_index]['font'])
s_theme_drop = Dropdown(parent=s_cont, x=100, y=45, options=['Green', 'Light', 'Dark'], color=global_themes[theme_index]['dark'])
s_refresh_label = Label(parent=s_cont, x=5, y=85, text='Automatic refresh: ', font=lv.font_montserrat_22, text_color=global_themes[theme_index]['font'])
s_refresh_drop = Dropdown(parent=s_cont, x=220, y=80, width=75, options=['Never', '30 sec', '1 min', '2 min'], color=global_themes[theme_index]['dark'])
s_bright_label = alte.Label(parent=s_cont, x=5, y=125, text='Brightness: ', font=lv.font_montserrat_22, text_color=global_themes[theme_index]['font'])
s_bright = BrightnessSlider(parent=s_cont, x=60, y=175, color=global_themes[theme_index]['font'], show_label=False)
s_export = Button(parent=s_cont, x=85, y=205, text='Export Data', color=global_themes[theme_index]['dark'], height=35, width=150, font=lv.font_montserrat_22)
s_save = Button(parent=s_cont, x=110, y=250, text='Save', color=global_themes[theme_index]['dark'], height=35, width=100, font=lv.font_montserrat_22)
...
g_cont = Page(x=0, y=t.get_height(), height=LV_VER_RES - t.get_height(), width=LV_HOR_RES, color=global_themes[theme_index]['light'], radius=0)
g_drop = Dropdown(parent=g_cont, x=35, y=5, width=250, options=['BPS Temperature (C)', 'BPS Pressure (kPa)'], color=global_themes[theme_index]['dark'])
g_temp = Gauge(parent=g_cont, x=80, y=44, gauge_color=0xFF0000, length=160, initial_value=0, max_value=50)
g_pres = Gauge(parent=g_cont, x=80, y=44, gauge_color=0xFF0000, length=160, initial_value=900, min_value = 900, max_value=1100)
g_pres.set_hidden(True)
...
# ------------------------------------------------- #
# -------------------- GENERAL -------------------- #
widgets_with_text = [temp_l, pres_l, s_title, s_theme_label, s_refresh_label, s_bright_label] #s_units_label, s_units_F]
...
def refresh_reads(is_click = False):
    ...
    try:
        ...
        bps = None
        bps_alert = Alert('ENV3 unit might not be connected!', text_color=global_themes[theme_index]['font'], title=lv.SYMBOL.WARNING + " Warning!", title_color=0xFF0000,
                  color=global_themes[theme_index]['light'])
        return
    ...
    print(str('Readings: Temp = ' + str(rounded_temp) + units + ', Pressure = ' + str(rounded_pres) + 'hPa' + ', Humidity = ' + str(rounded_humid) + 'RH'))
