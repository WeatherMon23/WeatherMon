...
def reset_charts(new_temp, new_pres):
    ...
    #temp_chart.set_points(temp_reads)
    #pres_chart.set_points(pres_reads)
...
def refresh_handler(source,evt):
    ...
    if evt == lv.EVENT.CLICKED:
        try:
            ...
            bps = None
            bps_alert = Alert('BPS unit might not be connected!', title=lv.SYMBOL.WARNING + " Warning!", title_color=0xFF0000,
...
m_cont = Container(x=0, y=t.get_height(), height=LV_VER_RES - t.get_height(), width=LV_HOR_RES, color=global_themes[theme_index]['light'], radius=0)
...
s_cont = Container(x=0, y=t.get_height(), height=LV_VER_RES - t.get_height(), width=LV_HOR_RES, color=global_themes[theme_index]['light'], radius=0)
...
s_refresh_drop = Dropdown(parent=s_cont, x=220, y=80, width=75, options=['2 min', '5 min', '10 min', '20 min'], color=global_themes[theme_index]['dark'])
s_save = Button(parent=s_cont, x=110, y=165, text='Save', color=global_themes[theme_index]['dark'], height=35, width=100, font=lv.font_montserrat_22)
...
g_cont = Container(x=0, y=t.get_height(), height=LV_VER_RES - t.get_height(), width=LV_HOR_RES, color=global_themes[theme_index]['light'], radius=0)
